...
namespace Kato
{
    public sealed class AppModel : PropertyChangedBase
    {
        ...
        public bool AutoStartOnStartup
        {
            ...
            set
            {
                if (value == m_autoStart) return;
                AutoStartButtonText = value ? "Disable AutoStart" : "Enable AutoStart";
                m_autoStart = value;
                NotifyOfPropertyChange(() => AutoStartOnStartup);
            }
        }


        string m_autoStartButtonText = "Enable AutoStart";
        public string AutoStartButtonText
        {
            get { return m_autoStartButtonText; }
            set
            {
                if (value == m_autoStartButtonText) return;

                m_autoStartButtonText = value;
                NotifyOfPropertyChange(() => AutoStartButtonText);
            }
        }
        ...
        {
            if (MessageBox.Show("Delete all settings?", "Confirmation", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.No)
                return;
            m_settings.Servers.Clear();
            m_servers.Clear();
            m_subscribedJobs.Clear();
            SaveSettings();            
        }
        ...
        {
            RegisterInStartup(!AutoStartOnStartup);
        }
        ...
        private bool IsAutoStartRegistered
        {
            get
            {
                ...
                var v = registryKey.GetValue("KatoBilgi");
                return v != null;    
            }
        }
        private void RegisterInStartup(bool isChecked)
        {
            ...
            if (isChecked)
            {
                ...
                registryKey.SetValue("KatoBilgi", appPath);
            }
            else
            {
                var v = registryKey.GetValue("KatoBilgi");
                if(v != null)
                    registryKey.DeleteValue("KatoBilgi");
            }

            AutoStartOnStartup = isChecked;
        }
        ...
        private void Update()
        {
            ...
            var subscribedJobs = m_servers.SelectMany(x => x.Jobs.Where(j => j.IsSubscribed))
                .OrderBy(j => (int)j.Status)
                .ThenByDescending(j=>j.LastBuild.TimeStamp)
        ...
        private void Initialize()
        {
            ...
            else
            {
                ...
                {
                    AddServer(server.DomainUrl, server.RequiresAuthentication,false);
                }
            }
            Update();
            SaveSettings();
        ...
        {
            var serverUris = servers.Cast<string>().Where(x => 
                !string.IsNullOrWhiteSpace(x) 
                && Uri.IsWellFormedUriString(x, UriKind.Absolute) 
                && !m_servers.Any(y => y.DomainUrl == x));

            foreach (string serverUri in serverUris)
                AddServer(serverUri,false,false);
        }
        ...
        private UserCredentials PromptForCredentials(string url, bool promptCreds)
        {
            UserCredentials result = null;
            CredentialsDialog dialog = new CredentialsDialog("Kato - Jenkins Authentication : \r\n" + url);
            dialog.AlwaysDisplay = promptCreds;
            dialog.Persist = true;
